...
describe('User', () => {
    ...
    describe('Verification Plugin', () => {
        ...
        it('should have a verification property', async() => {
            // create a new user
            let user = new User(userData);
            let savedUser = await user.save();

            expect(savedUser).to.have.property('isEmailVerified');
        });

        it('should issue a verification JWT', async() => {
            // create a new user
            let user = new User(userData);
            let savedUser = await user.save();

            // get a JWT
            expect(savedUser).to.have.property('getEmailVerificationToken');
            let jwt = savedUser.getEmailVerificationToken();
            expect(jwt.split('.')).to.have.lengthOf(3);
        });
        ...
        it('should issue a verification JWT', async() => {
            ...
            // translate the same JWT to get the user back
            expect(User).to.have.property('translateEmailVerificationToken');
            let user = await User.translateEmailVerificationToken(jwt);
            expect(user.id).to.equal(savedUser.id);
        });
    });
});
